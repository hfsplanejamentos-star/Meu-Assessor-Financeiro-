...
final class NotificationParser {
    ...
    static boolean looksFinancial(String title, String text) {
        ...
        return MONEY.matcher(value).find() && (OUT.matcher(value).find() || IN.matcher(value).find());
    }
    ...
    static Long extractAmountCents(String value) {
        ...
        try {
            return new BigDecimal(decimal).setScale(2, RoundingMode.HALF_UP).movePointRight(2).longValueExact();
