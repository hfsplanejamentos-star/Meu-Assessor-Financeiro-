...
function normalizeCore(){
 ...
 db.recurring.forEach(r=>{if(n(r.value)>0){r.value=-Math.abs(n(r.value));changed=true}if((String(r.id||'')==='rec_pensao'||/pensão alimentícia|pensao alimenticia|^pensão$|^pensao$/i.test(String(r.name||r.desc||'')))&&r.cat!=='Pensão'){r.cat='Pensão';r.desc='Pensão';r.description='Pensão';changed=true}if(String(r.id||'')==='rec_emp_mae'&&r.cat!=='Móveis'){r.cat='Móveis';r.desc='Móveis';r.description='Móveis';changed=true}if((String(r.id||'')==='rec_carro'||/prestação do carro|prestacao do carro/i.test(String(r.name||r.desc||'')))&&r.cat!=='C4 Cactus'){r.cat='C4 Cactus';changed=true}});
 return changed;
}
function ensurePlan(){
