...
(()=>{'use strict';
const FLAG='assessor_r68_initial_data_v1';
...
const baseline={
 ...
 transactions:[
  tx('salary_2026_10','2026-10-01','Salário outubro','Receitas',6500,{status:'planned'}),
  ...['2026-11-01','2026-12-01','2027-01-01','2027-02-01','2027-03-01','2027-04-01','2027-05-01','2027-06-01','2027-07-01'].map((d,i)=>tx('salary_'+d.replaceAll('-','_'),d,'Salário','Receitas',11500,{status:'planned'})),
  tx('13_2026','2026-12-01','13º salário proporcional estimado (4/12)','Receitas',4533.33,{status:'planned',estimated:true,note:'Estimativa bruta sobre salário-base de R$ 13.600; início informado em 11/09/2026.'})
 ]
};
function mergeById(base,items){const arr=Array.isArray(base)?base:[],ids=new Set(arr.map(x=>x&&x.id));for(const item of items)if(!ids.has(item.id)){arr.push(item);ids.add(item.id);}return arr;}
async function apply(){if(localStorage.getItem(FLAG)==='1')return;try{if(typeof db!=='object'||!db)return;db.accounts=mergeById(db.accounts,baseline.accounts);db.cards=mergeById(db.cards,baseline.cards);db.recurring=mergeById(db.recurring,baseline.recurring);db.transactions=mergeById(db.transactions,baseline.transactions);if(typeof save==='function')save();else if(typeof STORAGE!=='undefined')localStorage.setItem(STORAGE,JSON.stringify(db));localStorage.setItem(FLAG,'1');if(typeof renderAll==='function')renderAll();if(window.FinanceCloud?.configured?.())await window.FinanceCloud.push();console.info('[R6.8] Base financeira inicial aplicada.');}catch(e){console.error('[R6.8] Falha ao aplicar base inicial',e);}}
