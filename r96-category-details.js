...
(()=>{
 ...
 const monthFromTitle=()=>{const s=document.body.innerText.match(/(Janeiro|Fevereiro|Março|Abril|Maio|Junho|Julho|Agosto|Setembro|Outubro|Novembro|Dezembro) de (20\d{2})/i);if(!s)return null;const ms=['janeiro','fevereiro','março','abril','maio','junho','julho','agosto','setembro','outubro','novembro','dezembro'];return s[2]+'-'+String(ms.indexOf(s[1].toLowerCase())+1).padStart(2,'0')};
 ...
 function details(category){
  const month=(monthFromTitle()||document.querySelector('#monthSelect')?.value||document.querySelector('[data-month-select]')?.value||document.querySelector('.month-select')?.value||window.activeMonth||new Date().toISOString().slice(0,7)),now=new Date().toISOString().slice(0,7),future=month>now,rows=[];
