...
build=()=>{const next=legacyBuild(),c6=next.accounts.find(a=>a.id==='acc_c6'),caju=next.cards.find(c=>c.id==='card_caju_alimentacao');if(c6){c6.balance=486.31;c6.openingBalance=21.41;c6.balanceDate='2026-09-20'}if(caju){caju.limit=1006.89;caju.availableLimit=456.23;caju.balance=456.23;caju.excludeFromPatrimony=true}return next};
...
function migrateSep26(){
 ...
 if(db.meta.c6Sep2026Validated===version&&hasAll){
  ...
  const current=db.accounts.find(a=>a.id===aid);if(current){current.balance=486.31;current.openingBalance=21.41;current.balanceDate='2026-09-20';current.statementVerified=true}
